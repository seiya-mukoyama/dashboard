...
type FeedbackRecord = { date: string; from: string; comment: string; category: "technical" | "physical" | "mental" | "tactical" }
...
export function FeedbackHistory({ playerName }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/feedbacks?playerName=${encodeURIComponent(playerName)}`)
      .then(r => r.json()).then(data => setFeedbacks(Array.isArray(data) ? data : []))
        ...
        <div key={i} className="rounded-lg p-3 bg-secondary/50 border border-border space-y-1.5">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${categoryStyle[fb.category] ?? "bg-secondary text-muted-foreground"}`}>{categoryLabel[fb.category] ?? fb.category}</span>
