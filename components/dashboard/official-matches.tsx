...
export function OfficialMatches() {
  ...
  if (matches.length === 0) return <div className="flex items-center justify-center h-32 text-muted-foreground text-sm">公式戦データがありません</div>
  ...
  if (selected !== null) {
    ...
    const halvesOpp = m.halves.map(h => ({
      ...
      crosses: h.opp_crosses, shots: h.opp_shots,
      corners: h.opp_corners, freeKicks: h.opp_freeKicks,
    }))
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="rounded-xl border border-border bg-card p-5">
          <div className="text-xs text-muted-foreground text-center mb-3">{m.tournament} · {m.date}</div>
          {scoreTabs.length > 1 && (
            <div className="flex justify-center gap-1.5 mb-4 flex-wrap">
              {scoreTabs.map(tab => (
                <button key={tab} onClick={() => setScoreTab(tab)}
                  className={`px-3 py-1 rounded-full text-xs font-semibold transition-colors ${
                    scoreTab === tab ? 'bg-primary text-primary-foreground' : 'bg-secondary text-muted-foreground hover:text-foreground'
                  }`}>
                  {tab}
                </button>
              ))}
    ...
    <div className="space-y-3">
        ...
        {months.map(month => (
          <button key={month} onClick={() => setActiveMonth(month)}
            className={`px-4 py-1.5 rounded-full text-sm font-semibold transition-colors ${
              activeMonth === month ? 'bg-primary text-primary-foreground' : 'bg-secondary text-muted-foreground hover:text-foreground hover:bg-secondary/80'
            }`}>
            {month}
          </button>
        ))}
      ...
      <div className="space-y-2">
        {filtered.map((m, i) => {
          ...
          return (
            <button key={i} onClick={() => handleSelect(m, globalIdx)}
              ...
              <div className="flex-shrink-0 w-24">
                ...
                <div className="text-xs font-medium text-primary mt-0.5">{m.tournament}</div>
              </div>
